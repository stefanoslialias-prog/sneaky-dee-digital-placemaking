-- ✅ Fix RLS for Registration & Opt-in: Comprehensive Audit and Repair
...
BEGIN
  ...
  FOR r IN
    SELECT polname, schemaname, tablename, polcmd, roles, qual, with_check
    FROM pg_policies
  ...
  LOOP
    RAISE NOTICE 'Policy: % on %.% - CMD: % - USING: % - WITH CHECK: %', 
      r.polname, r.schemaname, r.tablename, r.polcmd, r.qual, r.with_check;
  END LOOP;
END $$;
...
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE 'Dropped policy: % on %.%', r.polname, r.schemaname, r.tablename;
  END LOOP;
END $$;
